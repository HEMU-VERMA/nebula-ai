...
class IdentityMutator:
    ...
    def mutate(self, candidate: Candidate, rng: random.Random) -> Candidate:
        ...
        del rng
        return Candidate(candidate_id(candidate.source, candidate.candidate_id), candidate.source, candidate.candidate_id)

class EvolutionEngine:
    """Run deterministic selection and mutation with user-supplied components."""
    def __init__(self, config: EvolutionConfig, mutator: Mutator, fitness: Fitness) -> None:
        self.config = config
    ...
    def run(self, initial: Candidate) -> EvolutionResult:
        ...
        for number in range(self.config.generations):
            ...
            survivor, _ = max(zip(population, current), key=lambda pair: pair[1].score)
        ...
        best = max(
            zip((item for generation in generations for item in generation.candidates), evaluations),
            key=lambda pair: pair[1].score,
            default=None,
        )
        return EvolutionResult(tuple(generations), tuple(evaluations), None if best is None else best[0])
