...
def test_engine_is_reproducible() -> None:
    ...
    first = EvolutionEngine(config, AppendMutator(), FixedFitness()).run(initial)
    second = EvolutionEngine(config, AppendMutator(), FixedFitness()).run(initial)
    assert first == second
